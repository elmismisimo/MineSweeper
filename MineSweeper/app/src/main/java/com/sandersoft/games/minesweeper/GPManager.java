...
public class GPManager implements Game, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public void defineScore(BoardController.Type type, long score){
        if (type == BoardController.Type.Custom) return;
        if (mGoogleApiClient.isConnected()) {
            String selectedScore = context.getResources().getString(
                    type == BoardController.Type.Easy ? R.string.score_easy :
                            type == BoardController.Type.Medium ? R.string.score_medium :
                                    R.string.score_hard);
